...
class DockerRuntime(ExecutionEnvironment):
    ...
    def _start_kubernetes_pod(
        ...
        try:
            ...
            if e.status == 404:
                self.logger.info(f"📝 Pod '{pod_name}' doesn't exist, will create new one")
            else:
                self.logger.error(f"❌ Error checking pod existence: {e.status} - {e.reason}")
                raise
        ...
        try:
            ...
            if not nodes.items:
                ...
                all_nodes = self.client.list_node(_request_timeout=30)
                for node in all_nodes.items[:5]:  # Show first 5
                    labels = node.metadata.labels
                    self.logger.error(f"  - {node.metadata.name}: {labels.get('sched-instance-type', 'no-label')}")
                raise RuntimeError("No matching nodes for pod scheduling")
            else:
                self.logger.info(f"✅ Found {len(nodes.items)} node(s) with matching label")

                # Check resource availability
                for node in nodes.items[:3]:  # Check first 3 nodes
                    allocatable = node.status.allocatable
                    self.logger.info(f"  Node {node.metadata.name}: CPU={allocatable.get('cpu')}, Memory={allocatable.get('memory')}")
        except Exception as e:
            self.logger.warning(f"⚠️ Could not validate cluster resources (continuing anyway): {e}")

        # Step 3: Prepare pod spec
        self.logger.info(f"📦 Preparing pod spec for image: {docker_image}")
        ...
        # Step 4: Create pod with detailed error handling
        self.logger.info(f"🚀 Creating pod '{pod_name}' in namespace '{DEFAULT_NAMESPACE}'...")
        max_retries = 3
        backoff = 5
        pod = None
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                self.logger.error(f"❌ Pod creation failed (attempt {attempt}/{max_retries})")
                self.logger.error(f"   Status: {e.status}")
                self.logger.error(f"   Reason: {e.reason}")
                self.logger.error(f"   Body: {e.body}")

                # Parse error message for common issues
                if e.status == 403:
                    self.logger.error("   🔐 RBAC ISSUE: Service account lacks permissions to create pods")
                    self.logger.error("   Fix: Grant pod-creator role to training pod's service account")
                elif e.status == 409:
                    ...
                    try:
                        ...
                        pass
                elif "insufficient" in str(e.body).lower() or "quota" in str(e.body).lower():
                    self.logger.error("   💾 RESOURCE ISSUE: Insufficient cluster resources")
                    self.logger.error("   Fix: Scale up node pool or reduce parallel pod count")
        ...
        if not pod:
            raise RuntimeError(f"Pod creation returned None for '{pod_name}'")

        # Step 5: Wait for pod to be Running (existing logic)
        self.logger.info(f"⏳ Waiting for pod '{pod_name}' to reach Running state...")
        try:
            w = watch.Watch()
            stream = w.stream(
            ...
            for event in stream:
                ...
                self.logger.info(f"   Pod status: {phase} (elapsed: {elapsed:.0f}s)")

                # Log container status details
                if obj.status.container_statuses:
                    for cs in obj.status.container_statuses:
                        if cs.state.waiting:
                            self.logger.warning(f"   Container waiting: {cs.state.waiting.reason} - {cs.state.waiting.message}")
                        elif cs.state.terminated:
                            self.logger.error(f"   Container terminated: {cs.state.terminated.reason} - {cs.state.terminated.message}")

                if phase == "Running":
                    self.logger.info(f"✅ Pod '{pod_name}' is Running (took {elapsed:.0f}s)")
                    w.stop()
                    break
                    ...
                    try:
                        ...
                        pass
                    raise RuntimeError(f"Pod '{pod_name}' failed with phase '{phase}'")

                if elapsed > 300:
                    w.stop()
                    raise RuntimeError(f"Timeout waiting for pod '{pod_name}' (5min)")
